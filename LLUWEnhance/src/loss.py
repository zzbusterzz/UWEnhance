...
def L_TV(x, TVLoss_weight=1):
    ...
    # h_x = array[1]
    # w_x = array[2]
    # count_h =  (array[2]-1) * array[3]
    # count_w = array[2] * (array[3] - 1)

    # h_tv = tf.Variable( tf.reduce_sum(tf.pow((x[:,1:,:,:]-x[:,:h_x-1,:,:]),2)) )
    # w_tv = tf.Variable( tf.reduce_sum(tf.pow((x[:,:,1:,:]-x[:,:,:w_x-1,:]),2)) )
