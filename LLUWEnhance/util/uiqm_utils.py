...
def eme(x, window_size_x, window_size_y):
    """
    ...
    k1 = math.floor(x.shape[1]/window_size_x)
    k2 = math.floor(x.shape[0]/window_size_y)
    # weight
    w = 2./(k1*k2)
    blocksize_x = window_size_x
    blocksize_y = window_size_y
    y_val = blocksize_y*k2 
    x_val = blocksize_x*k1
    ...
    for l in range(k1):
        for k in range(k2):
            block = x[k*window_size_y:window_size_y*(k+1), l*window_size_x:window_size_x*(l+1)]
            max_ = np.max(block)
            min_ = np.min(block)
...
def _uism(x):
    ...
    r_eme = eme(R_edge_map, 10, 9)
    g_eme = eme(G_edge_map, 10, 9)
    b_eme = eme(B_edge_map, 10, 9)
    # coefficients
    lambda_r = 0.299
    lambda_g = 0.587
    lambda_b = 0.144
...
def _uiconm(x, window_size_x, window_size_y):
    """
    ...
    # if 4 blocks, then 2x2...etc.
    k1 = math.floor(x.shape[1]/window_size_x)
    k2 = math.floor(x.shape[0]/window_size_x)
    # weight
    w = -1./(k1*k2)
    blocksize_x = window_size_x
    blocksize_y = window_size_x
    # make sure image is divisible by window_size - doesn't matter if we cut out some pixels
    ...
    for l in range(k1):
        for k in range(k2):
            block = x[k*window_size_x:window_size_x*(k+1), l*window_size_x:window_size_x*(l+1), :]
            max_ = np.max(block)
            min_ = np.min(block)
            top = max_-min_
            bot = max_+min_
