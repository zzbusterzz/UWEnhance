...
class UWGAN(object):
    ...
    def __init__(self, sess, input_height=640, input_width=480, output_height=256, output_width=256,
        ...
        # build model
        if self.y_dim:
            y = tf.compat.v1.placeholder(tf.float32, shape=[self.batch_size, self.y_dim], name='y')
            print(y)
        ...
        self.water_inputs = tf.compat.v1.placeholder(tf.float32, shape=[self.batch_size] + self.D_image_dims, name='real_images')
        self.air_inputs = tf.compat.v1.placeholder(tf.float32, shape=[self.batch_size] + self.image_dims, name='air_images')
        self.depth_inputs = tf.compat.v1.placeholder(tf.float32,
        ...
        self.G_sum = tf.compat.v1.summary.image("G", self.G, max_outputs=200)

        self.d_loss_real = tf.compat.v1.reduce_mean(
        ...
        tf.compat.v1.summary.scalar("D_realdata", self.D)
        tf.compat.v1.summary.scalar("D_fakedata", self.D_)
        ...
        for epoch in xrange(config.epoch+1):
            ...
            for idx in xrange(0, (water_batch_idxs*config.batch_size), config.batch_size):
                ...
                print("Epoch: [%2d] [%4d/%4d] time: %4.4f, d_loss: %.8f, g_loss: %.8f" \
                      % (epoch, idx, water_batch_idxs * config.batch_size,
                ...
                print(epoch%self.save_epoch)
                if epoch%self.save_epoch == 0 and idx <= 0 : #and epoch != 0:
                # np.mod(epoch, self.save_epoch) == 0 and epoch != 0:
    ...
    def test(self, config):
        ...
        for idx in xrange(0, sample_batch_idxs):
            ...
            for img_idx in range(0, test_batch_size):
                ...
                try:
                    imageio.imwrite(out_name, sample_im)
                ...
                try:
                    imageio.imwrite(out_name2, sample_im2)
                    #imageio.save(out_name2, sample_im2)
    ...
    def wc_generator(self, z, image, depth):
        ...
        """
        with tf.compat.v1.variable_scope("wc_generator"):

            # water-based attenuation and backscatter
            with tf.compat.v1.variable_scope("g_atten"):
                eta_r = tf.compat.v1.get_variable(name='g_eta_r', shape=[1, 1, 1], dtype=tf.float32,
                                        initializer=tf.random_normal_initializer(mean=0.35, stddev=0.01))
                eta_g = tf.compat.v1.get_variable(name='g_eta_g', shape=[1, 1, 1], dtype=tf.float32,
                                        initializer=tf.random_normal_initializer(mean=0.015, stddev=0.01))
                eta_b = tf.compat.v1.get_variable(name='g_eta_b', shape=[1, 1, 1], dtype=tf.float32,
            ...
            with tf.compat.v1.variable_scope("g_vig"):
                A = tf.compat.v1.get_variable('g_amp', [1], initializer=tf.random_normal_initializer(mean=0.75, stddev=0.01))
                B = tf.compat.v1.get_variable('g_tol', [1], initializer=tf.random_normal_initializer(mean=0.9, stddev=0.01))

            with tf.compat.v1.variable_scope('g_h1_convg'):
                w = tf.compat.v1.get_variable('g_wg', [5, 5, h0_z.get_shape()[-1], 1],
                                    initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.02))
            h1_zg = tf.nn.conv2d(h0_z, w, strides=[1, 1, 1, 1], padding='SAME')
            h_g = lrelu(self.g_bn1(h1_zg))

            with tf.compat.v1.variable_scope('g_h1_convr'):
                wr = tf.compat.v1.get_variable('g_wr', [5, 5, h0_z.get_shape()[-1], 1],
                                     initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.02))
            h1_zr = tf.nn.conv2d(h0_z, wr, strides=[1, 1, 1, 1], padding='SAME')
            h_r = lrelu(self.g_bn3(h1_zr))

            with tf.compat.v1.variable_scope('g_h1_convb'):
                wb = tf.compat.v1.get_variable('g_wb', [5, 5, h0_z.get_shape()[-1], 1],
                                     initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.02))
    ...
    def wc_sample(self, z, image, depth):
        ...
        """
        with tf.compat.v1.variable_scope("wc_generator", reuse=True):

            # water-based attenuation and backscatter
            with tf.compat.v1.variable_scope("g_atten", reuse=True):
                eta_r = tf.compat.v1.get_variable(name='g_eta_r', shape=[1, 1, 1], dtype=tf.float32,
                                        initializer=tf.random_normal_initializer(mean=0.35, stddev=0.01))
                eta_g = tf.compat.v1.get_variable(name='g_eta_g', shape=[1, 1, 1], dtype=tf.float32,
                                        initializer=tf.random_normal_initializer(mean=0.015, stddev=0.01))
            ...
            with tf.compat.v1.variable_scope("g_vig", reuse=True):
                A = tf.compat.v1.get_variable('g_amp', [1], initializer=tf.random_normal_initializer(mean=0.75, stddev=0.01))
                B = tf.compat.v1.get_variable('g_tol', [1], initializer=tf.random_normal_initializer(mean=0.9, stddev=0.01))

            with tf.compat.v1.variable_scope('g_h1_convg', reuse=True):
                w = tf.compat.v1.get_variable('g_wg', [5, 5, h0_z.get_shape()[-1], 1],
                                    initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.02))
            h1_zg = tf.nn.conv2d(h0_z, w, strides=[1, 1, 1, 1], padding='SAME')
            h_g = lrelu(self.g_bn1(h1_zg))

            with tf.compat.v1.variable_scope('g_h1_convr', reuse=True):
                wr = tf.compat.v1.get_variable('g_wr', [5, 5, h0_z.get_shape()[-1], 1],
                                     initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.02))
            h1_zr = tf.nn.conv2d(h0_z, wr, strides=[1, 1, 1, 1], padding='SAME')
            h_r = lrelu(self.g_bn3(h1_zr))

            with tf.compat.v1.variable_scope('g_h1_convb', reuse=True):
                wb = tf.compat.v1.get_variable('g_wb', [5, 5, h0_z.get_shape()[-1], 1],
                                     initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.02))
            h1_zb = tf.nn.conv2d(h0_z, wb, strides=[1, 1, 1, 1], padding='SAME')
            h_b = lrelu(self.g_bn4(h1_zb))
    ...
    def read_depth(self, filename):
        ...
        if self.is_crop:
           # depth = scipy.misc.imresize(depthtmp, (self.output_height, self.output_width), mode='F')
            #depth = np.array(depthtmp * self.output_height).astype(np.float32)
            img = Image.fromarray(depthtmp, mode='F').resize(size=(self.output_height, self.output_width))
    ...
    def read_img(self, filename):
        ...
        imgtmp = Image.open(filename)
        # print(imgtmp.shape)

        if self.is_crop:
            
           # img = scipy.misc.imresize(imgtmp, (self.output_height, self.output_width, 3))
            #img = Image.fromarray(imgtmp).resize(size=(self.output_height, self.output_width))
            img = imgtmp.resize(size=(self.output_height, self.output_width))
    ...
    def read_depth_small(self, filename):
        ...
        if self.is_crop:
            #depth = scipy.misc.imresize(depthtmp, (self.output_height, self.output_width), mode='F')
            depth = Image.fromarray(depthtmp, mode='F').resize(size=(self.output_height, self.output_width))
    ...
    def read_depth_sample(self, filename):
        ...
        if self.is_crop:
            ...
            depth = Image.fromarray(depthtmp, mode='F').resize(size=(self.sh, self.sh))
            depth = np.array(depth).astype(np.float32)
            # depth = np.multiply(self.max_depth, np.divide(depth, depth.max()))
            # depth = np.divide(depth, depth.max())
    ...
    def read_img_sample(self, filename):
        ...
        if self.is_crop:
            ...
            img = imgtmp.resize(size=(self.sh, self.sw))
            img = np.array(img).astype(np.float32)
            return img
        else:
            img = np.array(imgtmp).astype(np.float32)
